...
class ThemeManager {
  ...
  static void initializeTheme(BuildContext context, {ThemeBase? theme}) {
    try {
      String applicationThemeMode = VariableUtilities.preferences.getString(LocalCacheKey.applicationThemeMode) ?? 'auto';
  ...
  static void switchTheme(BuildContext context, {required ThemeBase? newTheme}) {
    VariableUtilities.preferences.setString(LocalCacheKey.applicationThemeMode, (newTheme == null) ? 'auto' : (newTheme is LightTheme ? 'light' : 'dark'));
    // setting new theme.
